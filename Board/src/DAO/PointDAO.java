	...
	public ArrayList<PointHistoryBean> PointSearchList( PointPagingBean pdto, int page) { 
        ...
        try {
            ...
            preparedStatement.setInt(3, startNum);      	
       		...
       		String sql = "select B.rnum, B.id, B.point, B.flag, CASE WHEN FLAG=1 THEN '적립' WHEN FLAG=2 THEN '사용' END AS type , TO_CHAR(B.p_date, 'yyyy-MM-dd HH:mi') as p_date from ( SELECT rownum as rnum, A.id, A.point, A.flag, A.p_date FROM ( SELECT id, point, flag, p_date from point_history order by 1 asc, 4 desc ) A where rownum <= ? and A.flag = ? ) B where B.rnum >= ?";
    		preparedStatement = connection.prepareStatement(sql); // sql명령문 담아둠
    		preparedStatement.setInt(1, endNum);
...
public int PointListTotalCount() {
    ...
    try {
      connection = dataSource.getConnection();	      
      String sql = "select B.rnum, B.id, B.total_point, B.save, B.use from ( SELECT rownum as rnum, A.id, A.total_point, A.save, A.use FROM ( SELECT id, total_point,save,use from point order by 2 desc, 1 asc ) A where rownum <= ?  ) B where B.rnum >= ?";
      preparedStatement = connection.prepareStatement(sql);
      preparedStatement.setInt(1, endNum);
      preparedStatement.setInt(2, startNum);
      resultSet = preparedStatement.executeQuery();       
